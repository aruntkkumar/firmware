...
const uint8_t USIDHIGHREV[36] =   {0x5C, 0x07, 0x40, 
                                   0x5C, 0x90, 0x41, 
                                   0x5C, 0x10, 0x7F, 
                                   0x5C, 0x90, 0x00, 
                                   0x5C, 0x10, 0x01, 
...
void MIPITRANSFER1 (uint8_t k){         //For SSC1, UID is LOW
        SDO1_SetHigh();
        asm("NOP");
        SDO1_SetLow();
...
void MIPITRANSFER2 (uint8_t k){         //For SSC2, UID is LOW
        SDO2_SetHigh();
        asm("NOP");
        SDO2_SetLow();
        readDummy = USIDLOWREV[0];
...
void SSCtest (uint8_t CABAND){
    switch (CABAND){
        case 0:
            MIPITRANSFER1 (33); //State 10
            MIPITRANSFER2 (3);  //State 64 (F6 & F7 ON)
            break;
        case 1:
            MIPITRANSFER1 (3);  //State 64 (F6 & F7 ON)
            MIPITRANSFER2 (33); //State 10
            break;
        case 2:
            MIPITRANSFER1 (6);  //State 63 (F1 - F6 ON)
            MIPITRANSFER2 (30); //State 0 (F1 - F7 OFF)
            break;
        case 3:
            MIPITRANSFER1 (30); //ALL OFF
            MIPITRANSFER2 (6);  //F1 - F6 ON
            break;
        case 4:
            MIPITRANSFER1 (3);  //F6 & F7 ON
            MIPITRANSFER2 (3);  //F6 & F7 ON
            break;
        case 5:
            break;
        case 6:
            MIPITRANSFER1 (33); //State 10
            MIPITRANSFER2 (30); //ALL OFF
            break;
        case 7:
            MIPITRANSFER1 (30); //ALL OFF
            MIPITRANSFER2 (33); //State 10
            break;
        case 8:
            MIPITRANSFER1 (27); //F1 ON
            MIPITRANSFER2 (30); //ALL OFF
            break;
        case 9:
            MIPITRANSFER1 (24); //F2 ON
            MIPITRANSFER2 (30); //ALL OFF
            break;
        case 10:
            MIPITRANSFER1 (21); //F3 ON
            MIPITRANSFER2 (30); //ALL OFF
            break;
        case 11:
            MIPITRANSFER1 (18); //F4 ON
            MIPITRANSFER2 (30); //ALL OFF
            break;
        case 12:
            MIPITRANSFER1 (15); //F5 ON
            MIPITRANSFER2 (30); //ALL OFF
            break;
        case 13:
            MIPITRANSFER1 (12); //F6 ON
            MIPITRANSFER2 (30); //ALL OFF
            break;
        case 14:
            MIPITRANSFER1 (9);  //F7 ON
            MIPITRANSFER2 (30); //ALL OFF
            break;
        default:
            MIPITRANSFER1 (6);  //F1 - F6 ON
            MIPITRANSFER2 (6);  //F1 - F6 ON
            break;
    }
}
...
void ConfigureShiftRegister(void){
    ...
    CTLB_LAT = 0;                    // Should already be zero
    SCK_SetHigh();            // Clock is left low so start with high
    asm("NOP");
    SCK_SetLow();             // This shifts data into register (due to inversion)
    
    for (i=0; i<8; i++) {
        CTLB_LAT = ReverseReg & 0x01;
        SCK_SetHigh();
        asm("NOP");
        SCK_SetLow();        // This shifts data into register (due to inversion)
